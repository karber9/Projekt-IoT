...
def nacl_secret_key(monkeypatch):
    monkeypatch.setattr(settings, "NACL_SECRET_KEY", TEST_KEY_HEX)


def test_prepare_ws_outgoing_message_plain():
    with patch("core.ws_crypto.settings.ENCRYPT_WEBSOCKET_PAYLOADS", False):
...
def test_prepare_ws_outgoing_message_encrypted():
    with patch("core.ws_crypto.settings.ENCRYPT_WEBSOCKET_PAYLOADS", True):
